---
layout: default
title: "Opportunities"
---
# Opportunities

<strong style="color: #007acc;">In general</strong>

We are always seeking motivated students and young researchers to join our team. Feel free to contact me to discuss potential opportunities at: ludovic.keiser[at]univ-cotedazur.fr

<strong style="color: #007acc;">Currently</strong>

<div style="display: flex; align-items: center; background-color: #ffeb3b; padding: 20px; border: 2px solid #f57c00; border-radius: 5px; margin-bottom: 20px;">
    <img src="/images/exclamation_bis.png" alt="Alert Icon" style="width: 100px; height: auto; margin-right: 15px;">
    <div>
        <h2 style="margin: 0; color: #d32f2f;">PhD Opportunity</h2>
        <p style="margin: 5px 0;">Together with Xavier Noblin, we are recruiting <strong>one PhD student (experimentalist)</strong>, to begin in November 2026, focusing on the optical imaging of embolism in plants, and on their replication in biomimetic artifical microchannels, using microfluidics techniques via clean room microfabrication. 
            
For more information, contact us by email: ludovic.keiser[at]univ-cotedazur.fr</p>
    </div>
</div>


 <h2 style="margin: 0; color: #d32f2f;">Postdoc opportunities</h2>
 Motivated young researchers are encouraged to contact us. In addition to position openings in our group, we can collaborate to develop proposals for submission to various local, national, and international funding opportunities.


<h2 style="margin: 0; color: #d32f2f;">Internship opportunities</h2>
We are always happy to welcome Bachelor and Master students for internships (experimental, numerical or theoretical), contact us and we can discuss the perimeter of the internship that would suit best with the duration, your background and your interests.



